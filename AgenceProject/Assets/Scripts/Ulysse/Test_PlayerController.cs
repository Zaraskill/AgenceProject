...
public class TestPlayerController : MonoBehaviour
{
    ...
    void RotateOnColliderSide()
    {
        transform.rotation = Quaternion.Euler(Vector3.zero);
        if (colliderSide == 0)
            graphes.transform.rotation = Quaternion.Euler(0, 0, 180);

        else if (colliderSide == 1)
            graphes.transform.rotation = Quaternion.Euler(0, 0, 90);
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        ...
        AudioManager.instance.Play("player_" + rdm);
            ...
            {
                Debug.Log("enter sticky " + rb.velocity);
    ...
    void OnCollisionExit2D(Collision2D other)
    {
        ...
        if (playerState == PlayerState.moving && rb.velocity != Vector2.zero && isStuck && other.gameObject.tag == "StickyWall")
        {
            isStuck = false;
        }
        else if (other.gameObject.tag == "StaticWall")
