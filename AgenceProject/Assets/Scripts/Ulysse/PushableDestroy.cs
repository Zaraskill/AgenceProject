...
public class PushableDestroy : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        ...
        if (other.relativeVelocity.magnitude > 1f && other.relativeVelocity.magnitude <= 5f)
            AudioManager.instance.RandomPlayVolume("SFX_Brick_Metal_Impact_", true, 1, 6, 0.1f);
        else if (other.relativeVelocity.magnitude > 5f && other.relativeVelocity.magnitude <= 8f)
            AudioManager.instance.RandomPlayVolume("SFX_Brick_Metal_Impact_", true, 1, 6, 0.2f);
        else if (other.relativeVelocity.magnitude > 8f)
            AudioManager.instance.RandomPlayVolume("SFX_Brick_Metal_Impact_", true, 1, 6, 0.3f);
    ...
    IEnumerator SoundPlayLate()
    {
        ...
        AudioManager.instance.RandomPlay("SFX_Brick_Metal_Dissolution_", true, 1, 6);
    }
}